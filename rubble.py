...
class Rubble:
    ...
    def __init__(self, num_grid, amin, amax, q, Sigma_d, rho_m=1.6, **kwargs):
        ...
        self.Sigma_g = Sigma_d * 100       # total gas surface density, in units of g/cm^2
        self.Sigma_dot = 0.0               # accretion rate on the surface density
        self.alpha = 1e-2                  # alpha description
        self.T = 1500                      # temperature, in units of K
        self.c_s = 232019.48601720118      # gas sound speed, in units of cm/s
        self.Omega = 6.296043195485603e-06 # Keplerian orbital frequency, in units of 1/s (assume 0.1 AU)
        self.H = self.c_s / self.Omega     # gas scale height
        self.kwargs = kwargs
        ...
        self.distribute_solids()                              # based on Sigma_d, Sigma_g, alpha, H
        self.calculate_dv()                                   # based on T, Re(Sigma_g, alpha), c_s(T)
        ...
        self._update_solids()                                 # based on Mdot, Raccu, H, Z
    ...
    def reset(self):
        ...
        # vertically integrated dust surface density distribution per logarithmic bin of grain radius
        self.sigma.fill(0); self.dsigma_in.fill(0); self.dsigma_out.fill(0)
        self.Nk.fill(0); self.Na.fill(0); self.dN.fill(0); self.St.fill(0); self.H_d.fill(0);
        
        ## intermediate matrixes used in simulation
        self.dv.fill(0); self.h_ss_ij.fill(0); self.vi_fac.fill(0)
        self.kernel.fill(0); self.p_c.fill(0); self.p_f.fill(0); self.K.fill(0); self.L.fill(0)
        self.C.fill(0); self.gF.fill(0); self.lF.fill(0);
        self.M1.fill(0); self.M2.fill(0); self.M3.fill(0); self.M4.fill(0); 
    ...
    def distribute_solids(self):
        ...
        self.H = H
        alpha = self.kwargs.get('alpha', 1e-2)
        self.alpha = alpha
    ...
    def _user_setup(self, test='BMonly'):
        ...
        if test == 'BMonly':
            ...
            alpha = self.kwargs.get('alpha', 1e-3) 
            c_s = (self.cgs_k_B * T / self.cgs_mu)**0.5
            v_TM = c_s * np.sqrt(alpha * self.St)
            v_TM[self.St > 1] = c_s * np.sqrt(alpha / self.St[self.St > 1])
            self.dv += np.sqrt(v_TM**2 + v_TM[:, np.newaxis]**2)            
            self.calculate_kernels()                              # based on dv
        elif test == 'constK':
            # constant kernel
            self.dv.fill(1.0)
            self.geo_cs.fill(1.0)
    ...
    def powerlaw_fragmentation_coeff(self):
        ...
        xi = self.kwargs.get('xi', 1.833) # power law index of fragment distribution, ref: Brauer+2008
        if xi > 2:
            self.warn_func(f"The power law index of fragment distribution, {xi} > 2, which means most mass "
                           +f"goes to smaller fragments. Do you really want that?")
        chi = self.kwargs.get('chi', 1)   # mass excavated from the larger particle due to cratering
        if chi >= 10:
            self.warn_func(f"The amount of mass removed from the larger object due to cratering (happends when mass"
                           +f"ratio >= 10), in units of the smaller object's mass, {chi} is >= 10, and has been reduced"
                           +f" to 10), which means a complete fragmentation also happens to the larger object. "
                           +f"Do you really want that?")
        
        C_norm = np.zeros(self.Ng+2)
        tmp_Nk = np.tril(self.m_j**(-1.83+1), -1)                       # fragments of i into j
        C_norm[1:] = self.m[1:] / np.sum(tmp_Nk * self.m_j, axis=1)[1:] # how unit mass will be distributed
        frag_Nk = tmp_Nk * C_norm[:, np.newaxis]
        
        idx_m_dec = int(np.ceil(1/self.dlog_m))                         # how many grid points for a mass decade
        idx_i_too_large = self.mesh3D_i - self.mesh3D_j >= idx_m_dec    # find ij for cratering (m_i > m_j * 10)
        idx_j_too_large = self.mesh3D_j - self.mesh3D_i >= idx_m_dec    # find ij for cratering (m_j > m_i * 10)
        ...
        tmp_idx = idx_both_frag & idx_i_too_large
        self.gF[tmp_idx] = frag_Nk[self.mesh3D_j[tmp_idx], self.mesh3D_k[tmp_idx]] * (1 + chi)
        tmp_idx = idx_both_frag & idx_j_too_large
        self.gF[tmp_idx] = frag_Nk[self.mesh3D_i[tmp_idx], self.mesh3D_k[tmp_idx]] * (1 + chi)
        ...
        self.gF[tmp_idx] = frag_Nk[self.mesh3D_j[tmp_idx], self.mesh3D_k[tmp_idx]]
         ...
         # can't simply this by self.gF[idx_ij_same] *= 0.5 b/c some self.gF[i][j][k] = 0 when i == j
        ...
        self.lF[idx_j_too_large] *= chi * self.m[self.mesh3D_i[idx_j_too_large]] / self.m[self.mesh3D_j[idx_j_too_large]]
    ...
    def calculate_dv(self):
        ...
        self.T = T
        self.dv_BM = np.sqrt(8 * self.cgs_k_B * T * self.m_sum_ij / (np.pi * self.m_prod_ij))
        
        # turbulent relative velocities (Ormel & Cuzzi 2007)
        self.c_s = (self.cgs_k_B * self.T / self.cgs_mu)**0.5
        u_gas_TM = self.c_s * np.sqrt(3/2 * self.alpha)
        Re = self.alpha * self.Sigma_g * 2e-15 / (2 * self.cgs_mu)
        St_12 = 1/1.6 * Re**(-1/2)       
        ...
        epsilon = St_i[St_inter]/St_j[St_inter]; epsilon[epsilon>1] = 1/epsilon[epsilon>1]
        dv_TM[St_inter] = np.sqrt(St_star12) * np.sqrt((2*1.6 - (1+epsilon) + 2/(1+epsilon)*(1/(1+1.6)+epsilon**3/(1.6+epsilon))))
    ...
    def _update_solids(self):
        """ how to distribute update solids """
    ...
    def one_step_implicit(self, dt):
        ...
        if self.N2Sigma(self.dN) / self._Sigma_d > 1e-8:
            self.warn_func("Relative error too large: sum(dSigma(dN))/Sigma_d = "
                           +f"{self.N2Sigma(self.dN)/self._Sigma_d:.3e}."
                           +" Consider using smaller timestep (a higher resolution mass grid usually won't help).")
        ...
        if np.any(self.Nk < 0): 
            ...
            if np.any(self.Nk < 0):
                raise RuntimeError(f"Some surface number density values remain significantly negative:"
                                   ...
                                   +f"\nThis usually means a bad initial distribution, please try a different setup...")
    ...
    def enforce_mass_con(self):
        ...
        self.Nk *= (self._Sigma_d / self.N2Sigma(self.Nk))
        self.sigma = self.Nk * 3*self.m
        self.Na = self.Nk * 3
    ...
    def simulation(self, tlim, dt, out_dt, out_log=False):
        ...
        if out_log:
            ...
            spd.drop_all() # to make sure other loggers are dropped
